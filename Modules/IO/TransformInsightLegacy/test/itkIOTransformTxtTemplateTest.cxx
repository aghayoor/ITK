...
static int oneTest(const char *goodname,const char *badname)
{
  ...
  itk::ObjectFactoryBase::RegisterFactory(itk::TxtTransformIOFactoryTemplate<ScalarType>::New() );
...
secondTest()
{
  ...
  try
    ...
    itk::TransformFileReaderTemplate<ScalarType>::TransformListType *list;
    list = reader->GetTransformList();
    itk::TransformFileReaderTemplate<ScalarType>::TransformListType::iterator lit =
      list->begin();
    while ( lit != list->end() )
      {
      (*lit)->Print ( std::cout );
...
int itkIOTransformTxtTemplateTest(int argc, char* argv[])
{
  if (argc > 1)
    {
    itksys::SystemTools::ChangeDirectory(argv[1]);
    }
  int result1 =  oneTest<float>("Transforms_float.txt", "TransformsBad_float.txt" );
  int result2 =  secondTest<float>();

  int result3 =  oneTest<double>("Transforms_double.txt", "TransformsBad_double.txt" );
